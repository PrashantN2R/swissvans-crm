...
@section('title', 'Edit Quotation | Superadmin')
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            ...
            <div class="col-12">
                <form class="row" action="{{ route('superadmin.quotations.update', $quotation->id) }}" method="POST" id="quotationForm"
                    ...
                    <div class="card">
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-6 mb-2 {{ $errors->has('quote_date') ? 'has-error' : '' }}">
                                    ...
                                    <input type="date" class="form-control" id="quote_date" name="quote_date"
                                        placeholder="Enter Due Date" value="{{ old('quote_date', $quotation->quote_date) }}">
                                    @error('quote_date')
                                ...
                                <div class="col-lg-6 mb-2 {{ $errors->has('expiry_date') ? 'has-error' : '' }}">
                                    ...
                                    <input type="date" class="form-control" id="expiry_date" name="expiry_date"
                                        placeholder="Enter Due Date" value="{{ old('expiry_date', $quotation->expiry_date) }}">
                                    @error('expiry_date')
                                ...
                                <div class="col-lg-12 mb-2 {{ $errors->has('note') ? 'has-error' : '' }}">
                                    ...
                                    <textarea class="form-control" id="note" name="note" placeholder="Write Note here" rows="3">{{ old('note', $quotation->note) }}</textarea>
                                    @error('note')
                                    ...
                                    <table id="item" class="table table-sm">
                                        ...
                                        <tbody>
                                            @foreach ($quotation->items as $key => $item)
                                             <tr id="item-row0">
                                                <td><input type="text" name="item[{{ $key }}][item]" placeholder="Item"
                                                        class="form-control" id="item_{{ $key }}" value="{{ $item->item }}" required></td>
                                                <td><input type="text" name="item[{{ $key }}][unit]" placeholder="Unit"
                                                        class="form-control" id="unit_{{ $key }}" value="{{ $item->unit }}" required></td>
                                                <td><input type="number" name="item[{{ $key }}][rate]" value="{{ $item->rate }}" placeholder="Rate"
                                                        class="form-control" id="rate_{{ $key }}" required step="any"></td>
                                                <td><input type="number" name="item[{{ $key }}][quantity]"  value="{{ $item->quantity }}"placeholder="Quantity"
                                                        class="form-control" id="quantity_{{ $key }}" required step="any">
                                                </td>
                                                <td><input type="number" name="item[{{ $key }}][amount]" value="{{ $item->amount }}" placeholder="Amount"
                                                        class="form-control" id="amount_{{ $key }}" required step="any"></td>
                        ...
                        <div class="card-footer bg-dark text-white">
                            <div class="d-flex justify-content-between">
                                <span>Total</span>
                                <span id="total">₹ {{ number_format($quotation->items->sum('amount')) }}</span>
                            </div>
                        </div>
