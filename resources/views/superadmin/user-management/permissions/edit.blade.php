...
@section('title', 'Edit Permission | Superadmin')
@section('content')
        ...
        <div class="row">
            <div class="col-12">
                ...
                <form id="superadminForm" method="POST"
                    action="{{ route('superadmin.permissions.update', $permission->id) }}" enctype="multipart/form-data"
                    autocomplete="off">
                    @csrf
                    @method('PUT')
                    <div class="card">
                                    ...
                                    <select class="form-control" id="role" name="role[]" multiple="multiple">
                                        @foreach ($roles as $role)
                                            <option value="{{ $role->name }}"
                                                {{ in_array($role->name, old('role') ? (array) old('role') : $permission->roles->pluck('name')->toArray()) ? 'selected' : '' }}>
                                                {{ $role->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('role')
