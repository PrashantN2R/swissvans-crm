...
@section('title', 'Create Task | Superadmin')
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                <form id="taskForm" method="POST" action="{{ route('superadmin.tasks.store') }}"
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-6 mb-2 {{ $errors->has('due_date') ? 'has-error' : '' }}">
                                    ...
                                    <input type="date" class="form-control" id="due_date" name="due_date"
                                        placeholder="Enter Due Date" value="{{ old('due_date') }}">
                                    @error('due_date')
                                ...
                                <div class="col-lg-12 mb-2 {{ $errors->has('task') ? 'has-error' : '' }}">
                                    <label class="col-form-label" for="task">Task Description<span
                                            class="text-danger">*</span></label>
                                    <textarea class="form-control" id="task" name="task" placeholder="Write Task Description" rows="5">{{ old('task') }}</textarea>
                                    @error('task')
                                    ...
                                    <select name="priority" id="priority" class="form-select">
                                        ...
                                        @foreach ($priorities as $priority)
                                            <option value="{{ $priority }}"
                                                {{ old('priority') == $priority ? 'selected' : '' }}>{{ $priority }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('priority')
